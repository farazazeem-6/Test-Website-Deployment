...
class DualRangeSlider {
    ...
    bindEvents() {
        ...
        this.minThumb.addEventListener('touchstart', (e) => this.handleTouchStart(e, 'min'));
        this.maxThumb.addEventListener('touchstart', (e) => this.handleTouchStart(e, 'max'));
...
ModalNextBtn.addEventListener('click', function (e) {
    ...
    if (currentPage === 1) {
        ...
        const selectedTerminal = document.querySelector('.form-select')?.value || '';
        const totalPrice = document.querySelector('.total-amount-of-seat p span')?.innerText || '0';
        const selectedSeats = JSON.parse(localStorage.getItem("selectedSeats")) || [];
        const pricePerSeat = extractPrice(document.querySelector('.price')?.innerText || 'PKR 0');
